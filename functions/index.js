...
const cors = require("cors")({ origin: true }); // ✅ GLOBAL CORS
...
exports.createRazorpayOrder = functions.https.onRequest(async (req, res) => {
    cors(req, res, async () => {   // ✅ CORS WRAPPER
        try {
...
// ======================================================
// 🔹 3. Order Email Sending
// ======================================================
exports.sendOrderConfirmation = onRequest({ region: "us-central1" }, async (req, res) => {
    cors(req, res, async () => {   // ✅ CORS WRAPPER
        try {
            const { orderDetails } = req.body;
            if (!orderDetails) return res.status(400).send({ error: "Missing order details" });
...
// ======================================================
exports.sendContactFormConfirmation = onRequest({ region: "us-central1" }, async (req, res) => {
    cors(req, res, async () => {   // ✅ CORS WRAPPER
        try {
            const { formDetails } = req.body;
            if (!formDetails) return res.status(400).send({ error: "Missing form details" });
